 ...
 * @param {string} awsService - Aws service related
 * @param {RequestContext[] | null} requestContexts - array of
 * RequestContexts or null if only want to authenticate (rather than
 * authenticate and authorize against policies)
 * @return {undefined}
 */
function doAuth(request, log, cb, awsService, requestContexts) {
    ...
    if (requestContexts) {
        requestContexts.forEach(requestContext => {
            ...
            requestContext
                .setSignatureVersion(res.params.data.signatureVersion);
